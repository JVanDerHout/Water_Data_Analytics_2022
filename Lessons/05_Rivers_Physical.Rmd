...
library(EcoHydRology) #load first
library(tidyverse)
...
#stormflow = total discharge - baseflow
...
ggplot(EnoDischarge, aes(x = Date, y = Discharge)) +
  ...
  scale_y_log10() + #helps see things
  labs(x = "Year", y = "Discharge (cfs)")
#baseflow is dependent on groundwater
#when groundwater is low, baseflow is low

# calculate baseflow with the EcoHydRology package.
EnoDischarge_basesep_EcoHydro <- BaseflowSeparation(EnoDischarge$Discharge)

#bind two datasets back together
...
#are two baseflow methods equivalent?
ggplot(EnoDischarge, aes(x = Baseflow_lfstat, y = bt)) +
  geom_point(alpha = 0.3, size = 1) +
  geom_abline(slope = 1, intercept = 0, color = "darkred")
#more similar at smaller flows
#more departures at higher flows 
#divergence is from event-based flows 

# How would we test that statistically?
#one way to do this - linear model
summary(lm(data = EnoDischarge, bt ~ Baseflow_lfstat))
#ideally intercept is 0
#you can also do a paired t-test
...
```{r}
#how much baseflow contributes to flow in the eno across the years
#using EcoHydRology package
...
EnoDischarge_summary <- EnoDischarge %>%
 ...
 mutate_if(is.numeric, round, 2) #rounds numeric column values to 2 decimal places
#remember to evaluate 1st and last year for missing data

ggplot(EnoDischarge_summary, aes(x = WaterYear, y = Discharge.acft.yr)) +
  geom_line() +
  geom_line(aes(y = Baseflow.acft.yr), lty = 2) #this changes line type
...
```{r}
#acquire discharge data and perform a baseflow separation analysis
...
attr(CoolidgeDischarge, "variableInfo")
attr(CoolidgeDischarge, "siteInfo")
...
CoolidgeDischarge$WaterYear <- as.numeric(as.character(CoolidgeDischarge$WaterYear))

CoolidgeDischarge_basesep_EcoHydro <- BaseflowSeparation(CoolidgeDischarge$Discharge)

CoolidgeDischarge <- cbind(CoolidgeDischarge, CoolidgeDischarge_basesep_EcoHydro)
...
ggplot(CoolidgeDischarge, aes(x = Date, y = Discharge)) +
  ...
  scale_y_log10() + 
  labs(x = "Year", y = "Discharge (cfs)")
#seeing effects of groundwater pumping on the oglala aquifer
```
