...
-Water is moving and is spatial 
-Elevation and flow lines
-Pollutant source, transport and fate
...
# filter only states that are included in the LAGOSNE database (only NE)
...
LAGOScombined <- 
  ...
  left_join(., LAGOSlimno) %>% #., means what you indicated above as the join
...
secchi.summary <- LAGOScombined %>%
  filter(sampledate > "2011-12-31") %>% #samples within the last 5 years
  group_by(lagoslakeid) %>%
  summarise(secchi.mean = mean(secchi), #taking the mean depth is an option, we can also look at best or worst conditions across the dataset 
            area = mean(lake_area_ha),
            depth = mean(maxdepth),
            lat = mean(nhd_lat), #we have to do something with them if we want to keep them in the dataframe but they will remain the same because there is only one 
            long = mean(nhd_long)) %>%
  drop_na()
```
...
ggplot() +
  ...
  geom_sf(data = secchi.spatial, aes(color = secchi.mean), #don't need to specify x and y  
          alpha = 0.5, size = 1) +
  scale_color_viridis_c(trans = "log10", end = 0.8, direction = -1, alpha = 0.5) +
  labs(color = "Average Secchi Depth (m)") +
  theme(legend.position = "top")

#can also be repeated with data on minimum or maximum secchi depth 
```
...
maine.secchi.summary <- MAINEcombined %>%
  filter(state == "ME") %>% 
  group_by(lagoslakeid) %>%
...
```{r}
maine.secchi.spatial <- st_as_sf(maine.secchi.summary, coords = c("long", "lat"), crs = 4326)

plot_a <- ggplot() +
  geom_sf(data = maine.subset, fill = "white") +
  geom_sf(data = maine.secchi.spatial, aes(color = secchi.mean), 
          alpha = 0.5, size = 1) +
  scale_color_viridis_c(trans = "log10", end = 0.8, direction = -1, alpha = 0.5) +
  labs(color = "Maine Average Secchi Depth (m)") +
  theme(legend.position = "top")
```
...
plot_b <- ggplot() +
  geom_sf(data = maine.subset, fill = "white") +
  geom_sf(data = maine.secchi.spatial, aes(color = secchi.mean), #change point size cex=depth ? 
          alpha = 0.5, size = 1) +
  scale_color_viridis_c(trans = "log10", end = 0.8, direction = -1, alpha = 0.5) +
  labs(color = "Maine Average Secchi Depth (m)") +
  theme(legend.position = "top")
