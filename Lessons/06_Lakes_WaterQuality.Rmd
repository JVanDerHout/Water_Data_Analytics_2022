...
options(scipen = 100) #sets report number of digits for exp notation

lagosne_get(dest_folder = LAGOSNE:::lagos_path(), overwrite = TRUE)
#downloads the database onto your machine
...
#data is loaded as a list, not a dataframe
#we have a lot of different dataframes within the list object
names(LAGOSdata)
...
#$ is used to pull out dataframes
...
#how many lakes are in each state? 
#bar chart does not require y value
ggplot(LAGOSlocations, aes(x = state)) + 
  geom_bar(stat = "count") #counts # of rows

#looking at median size of lakes per state
...
Trophic states relates to nutrients an net primary productivity.
High trophic states can result in a lot of respiration, reducing DO levels, impacting the food web. Cyanotoxins produced by HABs.
...
* *Secchi disk transparency*, a measure of water clarity. Pros: simple and cheap. Cons: may yield a high TSI in highly colored lakes and in lakes where particulate matter is comprised of non-algal material (multiple factors contributing)
...
Equations being used to do our calculations today
...
#way more samples in the summer months,  skews data a little 
...
#when TSI secchi is higher than chl reductions in clarity come from turbity
#when TSI chl is higher than secchi, reductions in clarity come from chlorophyll
#these conditions are across the year, no widespread difference in seasonality
...
ggplot(LAGOStrophic, aes(x = TSI.chl, y = TSI.tp, color = samplemonth)) +
  ...
  scale_color_viridis_c(option = "magma", end = 0.8, direction = -1) +
  theme(legend.position = "top")
#when TSI tp is higher than chl is when tp is not limiting primary production
#when TSI chl than tp 
...
ggplot(LAGOStrophic, aes(x = state, fill = trophic.class)) +
  geom_bar(stat = "count", position = position_dodge(preserve = "single")) + #preserve = single gives each variable the same width regardless of factor 
  theme(legend.position = "top") +
