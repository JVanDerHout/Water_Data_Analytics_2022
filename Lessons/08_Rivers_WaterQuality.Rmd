...
NeuseWQ <- readWQPqw(siteNumbers = "USGS-02089500", # Neuse River at Kinston, NC
                     ...
                     endDate = "")
#data retrieval package can be used to download data by type or by location
#note that the above is accessing from WQ portal and you have to add USGS- before site number
...
NeuseWQ_processed <- NeuseWQ %>% #creating easier to use dataset 
...
#to choose what is relevant to the water quality variables;
#filter based on flow between min and max of dataset if you are going to be combining datasets
...
#create a longer dataset (pivot_longer)
#long datasets allow for facet wrap to be done easily to create facets for each variable 
...
#what might cause that variability? 
...
```{r}
#many river trends are associated with flow
#mapping the two on top of one another by day of year 
...
#highest concentrations are during the dry season
...
#linear model to go with the above figure 
...
plot(lm(data = NeuseWQFlow_processed, log10(SpC_uS.cm) ~ log10(Flow_cfs)))
```
...
```{r}
NeuseFlowWQ_processed <- NeuseFlowWQ_processed %>% 
  select()

ggplot(NeuseFlowWQ_processed, aes(x = HydrologicCondition, y = FecalColiform_cfu.100mL))+
  geom_line()+
  facet_wrap(~HydrologicCondition)
