...
-morphology (sinuosity, cross sectional area)
-sediment type (channel roughness)
-slope
-discharge (what volume of water is moving and how quickly)
...
names(EnoDischarge.ratingcurve)[4:7] <- c("Discharge", "D.Approval.Code",
                                          "Gage.Height", "GH.Approval.Code") #renaming columns

ggplot(EnoDischarge.ratingcurve, aes(x = Discharge, y = Gage.Height)) +
  geom_point(alpha = 0.5) #as discharge increases, gage height increases consistently
#you can use this to estimate discharge from gage height which is what USGS is doing 
#rating curves are also used in restoration design 
...
This package is super helpful for figuring out what data can be accessed for a given site. 
...
#whatNWISdata can tell you what data you can access for a given site 
#parameter codes differ for each site 
```
...
```{r}
# Import data - last 10 years of data from each river to look at different discharge patterns
...
View(CombinedDischarge)
```

### Data Visualization
```{r}
# Build a ggplot - building one by one and giving them names
EnoPlot <- 
...
EnoPlot #date column is removed
...
#cowplot combined plot using plot_grid
CombinedPlot <- 
  plot_grid(EnoPlot, VerdePlot, BitterrootPlot, SaukPlot, NehalemPlot, 
            ncol = 1)
CombinedPlot
#plot grid can specify that some plots can be taller than others
#it does not automatically line up axes, so this needs to be specified
#have only one discharge label
#think about if you want to keep individual scales or have them all on one scale
# add relative heights and align axes
CombinedPlot <- 
  plot_grid(EnoPlot, VerdePlot, BitterrootPlot, SaukPlot, NehalemPlot, 
            ncol = 1, rel_heights = c(1,1,1,1.2), align = "hv") #fixes some alignment issues
CombinedPlot
```
...
> Verde River is an ephemeral river and is very responsive to precipitation events. 
...
BitterrootDischarge.Historical <- readNWISdv(siteNumbers = "12344000",
                     parameterCd = "00060", # discharge (ft3/s)
                     startDate = "")
#blank start and end date grabs whole period of record 

#renaming columns
names(BitterrootDischarge.Historical)[4:5] <- c("Discharge", "Approval.Code")

#specifying DOY in the dataset (adding new column)
BitterrootDischarge.Historical <- mutate(BitterrootDischarge.Historical, 
                                         DOY = yday(Date)) #yday is from lubridate

#make a summary across the years of data to see what a typical year looks like
#median, 95% and 5% to capture high and low (using quantile function)
#need to specify that you are removing NAs
...
#plot multiple lines with different y aes 
