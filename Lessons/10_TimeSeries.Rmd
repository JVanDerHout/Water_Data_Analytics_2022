...
> aggregation: Taking a timestep and using a central tendency or sum to go to higher or more spaced out timestep (daily -> monthly, monthly -> yearly, etc). Agregating might decrease temporal autocorrelation by looking at a larger scale. Some sampling schemes build this timescale thinking into them. 

> subsampling: Subsampling zooms into data. Many approaches require data subsampling - part of the data is used to build the model and part of the data is used to test the new set of data later. Another example is to keep the data at the same time-step but subset it at a different frequency within that time-step. 

> interpolation: Using surrounding data to fill in the gaps. 
...
* **Mann-Kendall**: no seasonality, non-parametric, no temporal autocorrelation, missing data allowed (equispaced not required). Function: `mk.test` (package: trend)
...
NeuseFlow_monthly <- NeuseFlow %>%
  mutate(Year = year(Date),
         Month = month(Date)) %>%
  group_by(Year, Month) %>%
  summarise(Discharge_acftmo = sum(Discharge)*1.98347) #this calculates acre feet per month 
...
#larger z (pos or neg) larger the p 
#s tracks the trends over time in seasonality and add the differences over time
#if trend is going up and down, s score will come close to 0
#if trend is strong in only one direction, S will show it 

```

What would we conclude based on these findings? 

> here we have a strong S trend meaning freshwater is getting more saline over time. because conductivity is our proxy for salinity. we can see that most significant increases are in winter months. therefore we are seeing increases in low conductivity over the year - baseline is getting higher rather than peaks getting higher over the year 
...
#we are switching from using interpolated data for conductivity to using agregated data for discharge
#we use the same test but might interpret these different kinds of data differently 
#parametrs are also in different units, so Z scores can be compared but S scores cannot 
```

What would we conclude based on these findings? 

> The significant changes in discharge are found in positive direction during the late summer / early fall months. This would mean that an increase in water quanitity would dilute the concentration. Increase in discharge is occuring over what is typically the low flow months. 
...
#the data are stationary over time
```
...
acf(NeuseFlow_timeseries) #determine autocorrelation
#autocorrelation decays as lag increases, but shows seasonality
pacf(NeuseFlow_timeseries) #determine moving average
#decays faster, not displaying much seasonality 
...
```{r}
#interpret them and decide which lag orders make sense to run with the data:
# run the arima function and search for best fit 
auto.arima(NeuseFlow_timeseries, trace = TRUE)
#use this to define autocorrelation and moving averages to make predictions into the future
#ARIMA(1,0,0)(0,0,2)[12] with non-zero mean : 14527.77

#run ARIMA on NeuseFlow
...
Neuseprediction <- predict(fit, n.ahead = 10*12) #10 years of 12 months each 

# plot future predictions -- but something is wrong!!! 
