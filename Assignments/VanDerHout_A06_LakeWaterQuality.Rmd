...
```{r}
NCdata$ActivityStartDate <- as.Date(NCdata$ActivityStartDate, format = "%Y-%m-%d")
...
NCdata.wrangled <- NCdata_processed %>% 
...
ActivityConductingOrganizationText, MonitoringLocationIdentifier, Depth_m, Variable) %>% 
  ...
  filter(Depth_m <= 1)
# got help on pivotwider 

#create a graph visualizing the chlorophyll-TP stressor-response relationship. Add a line of best fit for the linear regression, and adjust axis scales and labels as needed. 
class(NCdata.wrangled$TDP_mgL)
class(NCdata.wrangled$Chla_ugL)

ggplot(NCdata.wrangled, aes(x = TDP_mgL, y = Chla_ugL))+
  geom_point()+
