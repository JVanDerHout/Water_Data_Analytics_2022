...
```{r}
#2
NewHopeOWQ <- readWQPqw(siteNumbers = "USGS-02097314",
                        parameterCd = c("00300"))

NewHopeDO <- NewHopeOWQ %>% 
...
> Hypoxia seems to have occured early on in the dataset and not since the early 1980s. However, there may be an overall trend in decrease DO in the water column over time. 
...
TPplot <- ggplot(NewHopeTP, aes(x = Date, y = TP_mgL)) +
  geom_point() +
  labs(x = "Date", y = "Total Phosphorous")
...
> Nutrients are correlated with the increase and decrease of DO. Patterns in more variation in concentrations of phosphorous and dissolved oxygen are especially prevalent in the early 80s and lower and more stable levels of Phosphorous are mirrored in more stable and mid-range DO levels. This might indicate that higher levels of nutrient concentrationsare related to the occurence of hypoxia events. 
...
```{r}
NewHopeQ <- readNWISdv(siteNumbers = "02097314",
                        parameterCd = c("00060"))

NewHopeQ_processed <- NewHopeQ %>%
...
```{r}
QPlot80s <- ggplot(subset(NewHopeQ_processed, Date < "1985-01-01"), aes(x = Date, y = Flow_cfs)) +
  geom_point() +
  labs(x = "Date", y = "Flow (cfs)")


DOplot80s <- ggplot(subset(NewHopeDO, Date < "1985-01-01"), aes(x = Date, y = DO_mgL)) +
  geom_point() +
  geom_hline(yintercept = 2, lty = 2) + 
  labs(x = "Date", y = "Dissolved Oxygen (mg/L)")

plot_grid(DOplot80s, Qplot80s, ncol = 1)
#looked for help with modifying the dates
```

10. Do hypoxic events happen in the wet or the dry season? Why might that be?

> Hypoxic events occur in the dry season. This occurs because less water quantity means that water heats faster, and warmer water has a lower oxygen holding capacity. Additionally, warmer water may facilitate the growth of algae which will later die and contribute to further increase in biological oxygen demand through their decomposition. 
